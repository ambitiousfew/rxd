...
type Intracom[T any] interface {
	Register(ctx context.Context, conf TopicConfig) (*Topic[T], error)
...
type intracom[T any] struct {
	...
	// brokers hold a local cache of all topics, subscribers, and publishers.
	// and maintain the broadcaster routines for each topic.
	// NOTE: This is meant to replace the current broker channel
...
func (i *intracom[T]) Register(ctx context.Context, conf TopicConfig) (Topic[T], error) {
	i.mu.RLock()
	topic, ok := i.topics[conf.Topic]
	i.mu.RUnlock()
	if ok { // found the topic
		if conf.ErrIfExists { // if we should error if it already exists
			return topic, errors.New("topic " + conf.Topic + " already exists")
		}
		// return the existing topic instead.
		return topic, nil
	}
	// topic wasnt found, create a new one and return it.
	publishC := make(chan T)
	topic = NewTopic[T](publishC)
	i.mu.Lock()
	i.topics[conf.Topic] = topic
	i.mu.Unlock()
	return topic, nil
}
...
func (i *intracom[T]) Subscribe(ctx context.Context, conf SubscriberConfig) (<-chan T, func()) {
	...
	// i.mu.RLock()
	// broker, ok := i.brokers[cfg.Topic]
	// i.mu.RUnlock()
	// if !ok {

	// topic does not exist, register it
	// }
	return nil, func() {}
...
// 	case <-ctx.Done():
// 		return nil, false
// 	case i.requestC <- request: // send request
// 	}
...
func (i *intracom[T]) unsubscribe(ctx context.Context, topic, consumer string) func() {
	return func() {
		...
		request := unsubscribeRequest[T]{ // send request
			topic:     topic,
			consumer:  consumer,
			responseC: responseC,
		}
...
// 		request := unregisterRequest{ // send request to unregister topic
// 			topic:     topic,
// 			responseC: responseC,
// 		}
// 		select {
